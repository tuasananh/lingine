...
def main():
    ...
    for file_path, block in blocks.items():
        ...
        clean_block = "\n".join(block_lines[start_idx:end_idx]) + "\n"
        ...
        if file_path == "src/eval/defender_bonus.rs":
            ...
            if anchor_line_idx > 0 and "Tapered bonuses for having 0, 1, or 2 Advisors" in target_lines[anchor_line_idx - 1]:
                replace_start_line = anchor_line_idx - 1
        elif file_path == "src/eval/piece_square_tables.rs":
            # The line before is '#[rustfmt::skip]'
            if anchor_line_idx > 0 and "#[rustfmt::skip]" in target_lines[anchor_line_idx - 1]:
                replace_start_line = anchor_line_idx - 1
