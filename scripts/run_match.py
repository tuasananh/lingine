...
def check_dependencies(args):
    required_files = {
        ...
        args.openings: f"Opening database file",
    }
    
    if args.engine_a != "./target/release/lingine" and not os.path.exists(args.engine_a):
        required_files[args.engine_a] = "Engine A binary"
    if args.engine_b != "./target/release/lingine" and not os.path.exists(args.engine_b):
        required_files[args.engine_b] = "Engine B binary"
        
    utils.check_dependencies(required_files)

def main():
    parser = argparse.ArgumentParser(description="Lingine 2-Player Match Runner & ELO Evaluator")
    ...
    parser.add_argument("-g", "--games", type=int, default=1000, help="Total number of games (default: 1000)")
    parser.add_argument("-t", "--tc", default="3+0.03", help="Time control (default: 3+0.03)")
    parser.add_argument("-c", "--concurrency", type=int, help="Parallel games (default: auto)")
    parser.add_argument("-d", "--depth", type=int, default=12, help="Opening book ply depth (default: 12)")
    parser.add_argument("-f", "--openings", default="tools/xqdb_masters_40711_UCI_games.pgn", help="Path to openings file")
    parser.add_argument("-o", "--outdir", "--pgnout", help="Output directory")
    parser.add_argument("-s", "--skip-build", action="store_true", help="Skip cargo build")
    ...
    if not args.outdir:
        ...
        user_outdir = input(f"Enter match name or folder (Press Enter for default: {default_outdir}): ").strip()
        if not user_outdir:
            args.outdir = default_outdir
        else:
            args.outdir = f"matches/{user_outdir}" if "/" not in user_outdir else user_outdir
    ...
    if not args.skip_build and ("./target/release/lingine" in (args.engine_a, args.engine_b)):
        utils.build_engine()
    else:
    ...
    engine_a_args = ["-engine", f"cmd={args.engine_a}", f"name={args.name_a}", f"stderr={args.name_a}_err.log"]
    if args.options_a:
        engine_a_args.extend(args.options_a.split())
        
    engine_b_args = ["-engine", f"cmd={args.engine_b}", f"name={args.name_b}", f"stderr={args.name_b}_err.log"]
    if args.options_b:
        engine_b_args.extend(args.options_b.split())
        
    cli_args = [
        "./tools/sylvan-cli"
    ] + engine_a_args + engine_b_args + [
        "-each", "proto=uci", f"tc={args.tc}", "option.Hash=16",
        "-openings", f"file={args.openings}", "format=pgn", f"plies={args.depth}",
        "-tournament", "round-robin", "-games", str(args.games), "-repeat",
        "-concurrency", str(concurrency), "-pgnout", pgnout
    ]
    ...
    try:
        ...
        subprocess.run([sys.executable, "scripts/analyze_match.py", pgnout, "-m", os.path.join(args.outdir, "summary.md")], check=True)
