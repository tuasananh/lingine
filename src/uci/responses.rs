...
pub enum UciOption {
    ...
    /// An empty string serialises the special token `<empty>` per spec.
    /// Serialises as: `option name <name> type string default <value|<empty>>`
...
///
/// All fields are optional; only fields that are `Some` are serialised.
/// Per spec, if `seldepth` is present then `depth` must also be present.
#[derive(Default)]
pub struct UciInfo {
