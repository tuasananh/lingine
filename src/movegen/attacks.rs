...
mod tests {
    ...
    fn test_rook_attacks() {
        ...
        for f in 1..9 {
            expected_mask |= 1u128 << (0 * 9 + f);
        }
        for r in 1..10 {
            expected_mask |= 1u128 << (r * 9 + 0);
        }
        assert_eq!(att_a0.0, expected_mask);

        // Rook at A0, blocked by a piece at A2 and B0
        let blocked = Bitboard((1u128 << (2 * 9 + 0)) | (1u128 << (0 * 9 + 1)));
        ...
        expected_blocked_mask |= 1u128 << (0 * 9 + 1); // B0
        expected_blocked_mask |= 1u128 << (1 * 9 + 0); // A1
        expected_blocked_mask |= 1u128 << (2 * 9 + 0); // A2
        assert_eq!(att_a0_blocked.0, expected_blocked_mask);
    }
    ...
    fn test_cannon_attacks() {
        ...
        let occupied = Bitboard((1u128 << (2 * 9 + 0)) | (1u128 << (5 * 9 + 0)));
        ...
        expected |= 1u128 << (0 * 9 + 3); // D0
        expected |= 1u128 << (0 * 9 + 5); // F0
        expected |= 1u128 << (1 * 9 + 4); // E1
        assert_eq!(att_cannon_blocked.0, 1u128 << (5 * 9 + 0));
        assert_eq!(KING_ATTACKS[Square::E0 as usize].0, expected);
        ...
        expected_adv |= 1u128 << (0 * 9 + 3); // D0
        expected_adv |= 1u128 << (0 * 9 + 5); // F0
        expected_adv |= 1u128 << (2 * 9 + 3); // D2
        expected_adv |= 1u128 << (2 * 9 + 5); // F2
        assert_eq!(advisor_e1.0, expected_adv);
    }
    ...
    fn test_bishop_blocking() {
        ...
        expected_blocked |= 1u128 << (2 * 9 + 0); // A2 is still valid
        assert_eq!(blocked_attacks.0, expected_blocked);
    }
}
