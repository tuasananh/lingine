...
class Matchmaking:
    def __init__(self, li: Lichess, config, username):
        ...
        self.variants = list(
            filter(
                lambda variant: variant != "fromPosition",
                config["challenge"]["variants"],
            )
        )
    ...
    def should_create_challenge(self):
        ...
        time_has_passed = (
            ...
            + ((self.matchmaking_cfg.get("challenge_timeout") or 30) * 60)
            < time.time()
        )
        challenge_expired = (
            self.last_challenge_created + self.challenge_expire_time < time.time()
            and self.challenge_id
        )
        ...
        online_bots = list(
            filter(
                lambda bot: (
                    ...
                    and min_rating
                    <= ((bot["perfs"].get(game_type) or {}).get("rating") or 0)
                    <= max_rating
                ),
                online_bots,
            )
