...
class Lichess:
    ...
    def get_event_stream(self):
        ...
        logger.debug(f"Opening event stream from {url}")
    ...
    def get_game_stream(self, game_id):
        ...
        logger.debug(f"Opening game stream for {game_id} from {url}")
        return requests.get(url, headers=self.header, stream=True)
    ...
    def cancel(self, challenge_id):
        return self.api_post(
            ENDPOINTS["cancel"].format(challenge_id), raise_for_status=False
        )
