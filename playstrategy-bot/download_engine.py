    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith("engine:"):
                ...
                continue
            elif in_engine_section and not line.startswith(" ") and not line.startswith("\t"):
                in_engine_section = False
...
def main():
    ...
    req = urllib.request.Request(
        API_URL, 
        headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) lingine-updater"}
    )
    ...
    if not download_url:
        print("Error: Could not find a linux-x86_64 asset in the latest release.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        with urllib.request.urlopen(download_req) as response, open(engine_path, "wb") as out_file:
